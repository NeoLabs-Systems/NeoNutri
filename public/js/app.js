...
/* ═══════════════════════════════════════════════════════════════
   NeoNutri — Frontend Application
   ═══════════════════════════════════════════════════════════════ */

const App = (() => {

  // ─── State ─────────────────────────────────────────────────────
  let token         = localStorage.getItem('nn_token') || null;
  ...
  function openSettings(tab) {
    ...
    else switchSettingsTab('profile');
    loadProfile();
  }
  ...
  // ─── Tab Switching ─────────────────────────────────────────────
  const TAB_TITLES = { dashboard: 'Today', fasting: 'Fasting', charts: 'Progress', profile: 'Profile', chat: 'Chat', camera: 'Log Food' };
  ...
  function switchTab(tab) {
    ...
    if (titleEl) titleEl.textContent = TAB_TITLES[tab] || 'NeoNutri';
  ...
  function renderProfile(u) {
    ...
    const sAvatar = document.getElementById('s-profile-avatar');
    const sName   = document.getElementById('s-profile-name');
    const sEmail  = document.getElementById('s-profile-email');
    if (sAvatar) sAvatar.textContent = initials;
    if (sName)   sName.textContent   = u.name  || 'User';
  ...
  async function saveProfile() {
    ...
    const btn = document.getElementById('save-profile-btn') || document.querySelector('[onclick="App.saveProfile()"]');
  ...
  function showNotifPrompt() {
    ...
    if (!el) return;
    const dismissed = localStorage.getItem('nn_notif_dismissed');
  ...
  function dismissNotifPrompt() {
    ...
    localStorage.setItem('nn_notif_dismissed', '1');
  }
  ...
  async function requestNotifications() {
    ...
    if (perm === 'granted') {
      ...
      scheduleReminders();
      sendNotification('NeoNutri reminders on', 'You\'ll get daily meal and fasting reminders.');
    } else {
      localStorage.setItem('nn_notif_dismissed', '1');
        ...
        return setTimeout(async () => {
          try {
            ...
            if ((totals.calories || 0) < 200) {
              sendNotification('Don\'t forget to log your meals!', 'Tap to open NeoNutri and track your nutrition.');
            }
          } catch {}
        }, ms);
      }
